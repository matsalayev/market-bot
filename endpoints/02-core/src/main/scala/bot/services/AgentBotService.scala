...
trait AgentBotService[F[_]] {
  def telegramMessage(update: Update): F[Unit]
}

object AgentBotService {
  def make[F[_]: Monad: GenUUID: Calendar: Sync](
      ...
      logger: Logger[F]
    ): AgentBotService[F] = new AgentBotService[F] {
    ...
    private def handleTextMessage(user: User, text: String): F[Unit] =
      text match {
        case "/start" => telegramClient.sendMessage(user.id, "Welcome to the Agent Bot!")
